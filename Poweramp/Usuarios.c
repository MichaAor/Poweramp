...
/*
///CARGCAR PLAYLIST
void cargarPlaylist(int idcan,int idUs){
    stUser a;

    int flag=0;
    FILE*archiUsuario=fopen(arUser,"r+b");
    if(archiUsuario!=NULL){
        while(flag==0&&fread(&a,sizeof(stUser),1,archiUsuario)>0){
            if(a.idUsuario==idUs){
                a.playlist[a.validosPlaylist]=idcan;
                a.validosPlaylist++;
                fseek(archiUsuario,sizeof(stUser)*(-1),SEEK_CUR);
                fwrite(&a,sizeof(stUser),1,archiUsuario);
                flag=1;
            }
        }
